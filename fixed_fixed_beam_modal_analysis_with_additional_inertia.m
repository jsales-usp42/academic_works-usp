...
I_ad = 0.5;            % inércia rotacional concentrada no meio (valor arbitrário)
...
% Montagem da matriz de inércia [I] com inércia adicional no meio
I_rot = rho * dx;        % inércia rotacional de cada elemento
I = I_rot * eye(N);      % matriz diagonal

% Adicionando inércia concentrada no meio do vão
meio = round(N / 2);     % índice do grau de liberdade central
I(meio, meio) += I_ad;   % soma da inércia adicional
...
% Solução exata contínua (sem inércia adicional, para comparação)
n_modes = min(5, N);
omega_analitico = (1:n_modes)' * pi;
f_analitico = omega_analitico / (2*pi);

% Exibindo os resultados
disp('Modo   omega_numérico (rad/s)   f_numérico (Hz)   omega_analítico (rad/s)   f_analitico (Hz)');
for i = 1:n_modes
...
for mode = 1:n_modes
    ...
    title(['Modo ', num2str(mode), ...
        ' - \omega = ', num2str(omegas(mode), '%.4f'), ' rad/s com I_{ad}']);
